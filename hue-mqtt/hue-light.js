    ...
    function HueLight(config) {
        ...
        if (bridge == null) {
            this.status({
                fill: "red",
                shape: "ring",
                text: "hue-light.node.not-configured",
            });
            return false;
        }
            ...
            lights
                .then((lights) => {
                    lights.forEach((light) => {
                        if (
                            lightName !== undefined &&
                            lightName.toLowerCase() !== light.name.toLowerCase()
                        ) {
                            return;
                        }
                        ...
                        if (payload.brightness !== undefined) {
                            if (
                                ...
                                node.error(
                                    RED._(
                                        "hue-light.node.error-invalid-brightness"
                                    )
                                );
                                return false;
                            }
                        ...
                        if (
                            ...
                            if (
                                ...
                                node.error(
                                    RED._(
                                        "hue-light.node.error-invalid-saturation"
                                    )
                                );
                                return false;
                            }

                            light.saturation = Math.round(
                        ...
                        if (
                            ...
                            if (colorTemp < 153 || colorTemp > 500) {
                                node.error(
                                    RED._(
                                        "hue-light.node.error-invalid-color-temp"
                                    )
                                );
                                return false;
                            }
                        ...
                        if (changesCount) {
                            node.debug(
                                `${changesCount} change(s) required on light ${light}: ${JSON.stringify(
                                    light.state.changed
                                )}`
                            );
                ...
                .catch((error) => {
                    node.status({
                        fill: "red",
                        shape: "ring",
                        text: "hue-light.node.error-change-state",
                    });
                    node.error(error);
                ...
                if (config.light) {
                    if (!msg.payload.reachable) {
                        node.status({
                            fill: "grey",
                            shape: "ring",
                            text: "hue-light.node.unreachable",
                        });
                    } else if (msg.payload.on) {
                        node.status({
                            fill: "green",
                            shape: "dot",
                            text: "hue-light.node.on",
                        });
                    } else {
                        node.status({
                            fill: "red",
                            shape: "dot",
                            text: "hue-light.node.off",
                        });
                    }
                } else {
                    node.status({});
