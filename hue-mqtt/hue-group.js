    ...
    function HueGroup(config) {
        ...
        if (bridge == null) {
            this.status({
                fill: "red",
                shape: "ring",
                text: "hue-group.node.not-configured",
            });
            return false;
        }
            ...
            groups
                .then((groups) => {
                    groups.forEach((group) => {
                        if (
                            groupName !== undefined &&
                            groupName.toLowerCase() !== group.name.toLowerCase()
                        ) {
                            return;
                        }
                        ...
                        if (payload.brightness !== undefined) {
                            if (
                                ...
                                node.error(
                                    RED._(
                                        "hue-group.node.error-invalid-brightness"
                                    )
                                );
                                return false;
                            }
                            ...
                            if (payload.brightness === 0) {
                                ...
                                group.on = true;
                                group.brightness = Math.round(
                                    (254 / 100) * parseInt(payload.brightness)
                                );
                            }
                        }
                        ...
                        // Set saturation
                        if (
                            group.saturation !== undefined &&
                            payload.saturation !== undefined
                        ) {
                            if (
                                payload.saturation < 0 ||
                                payload.saturation > 100
                            ) {
                                node.error(
                                    RED._(
                                        "hue-group.node.error-invalid-saturation"
                                    )
                                );
                                return false;
                            }

                            group.saturation = Math.round(
                                (254 / 100) * parseInt(payload.saturation)
                            );
                        }

                        // Set color temperature
                        if (
                            group.colorTemp !== undefined &&
                            payload.colorTemp !== undefined
                        ) {
                            ...
                            if (colorTemp < 153 || colorTemp > 500) {
                                node.error(
                                    RED._(
                                        "hue-group.node.error-invalid-color-temp"
                                    )
                                );
                                return false;
                            }
                        ...
                        if (changesCount) {
                            node.debug(
                                `${changesCount} change(s) required on group ${group}: ${JSON.stringify(
                                    group.action.changed
                                )}`
                            );
                ...
                .catch((error) => {
                    node.status({
                        fill: "red",
                        shape: "ring",
                        text: "hue-group.node.error-input",
                    });
                    if (done) {
                        done(error);
                    }
                ...
                if (config.group) {
                    if (msg.payload.allOn) {
                        node.status({
                            fill: "green",
                            shape: "dot",
                            text: "hue-group.node.all-on",
                        });
                    }
                    if (msg.payload.anyOn) {
                        node.status({
                            fill: "green",
                            shape: "dot",
                            text: "hue-group.node.some-on",
                        });
                    } else {
                        node.status({
                            fill: "red",
                            shape: "dot",
                            text: "hue-group.node.all-off",
                        });
                    }
                } else {
                    node.status({});
