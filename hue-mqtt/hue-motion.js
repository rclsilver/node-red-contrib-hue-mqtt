                ...
                if (config.sensor) {
                    if (msg.payload.active) {
                        if (msg.payload.motion) {
                            node.status({ fill: 'green', shape: 'dot', text: 'hue-motion.node.motion-detected' });
                        } else {
                            node.status({});
                        }
                    } else {
                        node.status({ fill: 'red', shape: 'ring', text: 'hue-motion.node.disabled' });
                    }
                } else {
                    node.status({});
                }
        ...
        node.on('input', (msg, send, done) => {
            if ('payload' in msg) {
                ...
                if (sensor && config.sensor && sensor != config.sensor) {
                    node.status({ fill: 'red', shape: 'ring', text: 'hue-motion.node.cannot-override-sensor' });
                            ...
                            sensors
                                ...
                                .forEach(sensor => {
                                    ...
                                    bridge.client.sensors.save(sensor).then(sensor => {
                                        if (sensor.config.on) {
                                            node.debug(`Sensor ${sensor.name} (${sensor.id}) enabled`);
                                        } else {
                                            node.debug(`Sensor ${sensor.name} (${sensor.id}) disabled`);
                                        }

                                        bridge.sendDeviceUpdate('sensor', sensor);
                                    }).catch(error => {
                                        node.error(`Error while changing state of ${sensor.name}: ${error}`);
                                    });
                                });
                        });
                    } else {
                        bridge.client.sensors.getById(sensor).then(sensor => {
                            if (sensor.config.on != enabled) {
                                sensor.config.on = enabled;
                                return bridge.client.sensors.save(sensor);
                            } else {
                                return new Promise(resolve => resolve(sensor));
                            }
                        }).then(sensor => {
                            if (sensor.config.on) {
                                node.debug(`Sensor ${sensor.name} (${sensor.id}) enabled`);
