module.exports = function (RED) {
    function HueBridge(config) {
            ...
            return new Promise((resolve, reject) => {
                if (useCache) {
                    ...
                    if (device) {
                        ...
                        reject(
                            `Device ${type} with id ${id} not found in cache`
                        );
                    }
                } else {
                    node.client[type + "s"]
                        .getById(id)
                        .then(resolve)
                        .catch(`Device ${type} with id ${id} not found`);
                }
            });
        };
        ...
        node.poller.on("started", () => {
            node.log(
                `Starting polling with interval of ${node.poller.interval}ms`
            );
        });
        ...
        node.poller.on("error", (error) => {
            node.error(
                `Error while refreshing cache from the bridge: ${error}`
            );
        });
        ...
        this.refresh()
            ...
            .catch((error) => {
                ...
                node.error(
                    `Error while initializing cache from the Hue bridge: ${error}`
                );
            })
